...
static async Task ApplyMigrationsAsync(IServiceProvider services, ILogger logger)
{
    ...
    for (var attempt = 1; attempt <= maxAttempts; attempt++)
    {
        ...
        catch (Exception exception) when (attempt < maxAttempts)
        {
            logger.LogWarning(
                exception,
                "Failed to apply campaign migrations on attempt {Attempt}/{MaxAttempts}. Retrying in {DelaySeconds}s.",
                attempt,
                maxAttempts,
                delay.TotalSeconds);
    ...
    var finalDbContext = finalScope.ServiceProvider.GetRequiredService<CampaignDbContext>();
